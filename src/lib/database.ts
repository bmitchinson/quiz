...
export class Database {
	...
	async addTeacher(name: string, grade: number): Promise<void> {
		try {
			...
			logger.error('database', 'Error adding teacher:', error);
			throw error;
		}
	}
	...
	async getTeacher(lastname: string) {
		try {
			...
			logger.error('database', 'Error looking up teacher:', error);
			throw error;
		}
	}
	...
	async addStudents(studentNames: string[], teacherName: string) {
		try {
			...
			logger.error('database', 'Error adding students:', error);
			throw error;
		}
	}
	...
	async getStudentsOfTeacher(teacherId: int): Promise<string[]> {
		try {
			...
			logger.error('database', 'Error fetching students:', error);
			throw error;
		}
	}
	...
	async getSummaryOfScores(grade: number, teacherName: string, year = 2425) {
		try {
			...
			logger.error('database', 'Error getting a summary of scores:', error);
			throw error;
		}
	}
	...
	async getScores(filters: GetScoresFilters): Promise<GetScoresScore[]> {
		try {
			...
			logger.error('database', 'Error fetching scores:', error);
			throw error;
		}
	}
	...
	async archiveStudent(name: string, teacherId: int): Promise<void> {
		try {
			...
			logger.error('database', 'Error deleting student:', error);
			throw error;
		}
	}
	...
	async addQuiz(
		...
		try {
			...
			logger.error('database', 'Error adding quiz:', error);
			throw error;
		}
	}
	...
	async updateOrCreateScore(
		...
		try {
			...
			logger.error('database', 'Error adding score:', error);
			throw error;
		}
	}
	...
	async getQuiz(accessCode: string) {
		try {
			...
			logger.error('database', 'Error looking up quiz:', error);
			throw error;
		}
	}
	...
	async getQuizzesByAccessCodes(accessCodes: string[]): { [accessCode: string]: Quiz } {
		try {
			...
			logger.error('database', 'Error looking up quizzes for access codes:', error);
			throw error;
		}
	}
	...
	async getQuizByMetadata(year: number, grade: number, quarter: number, sequenceLetter: string) {
		try {
			...
			logger.error('database', 'Error checking if quiz exists:', error);
			throw error;
		}
	}
	...
	async deleteScore(scoreId: number): Promise<void> {
		try {
			...
			logger.error('database', `Error deleting score ${scoreId}:`, error);
			throw error;
		}
	}
	...
	async updateQuizQuestions(accessCode: number, questionsData: string): Promise<void> {
		try {
			...
			logger.error('database', 'Error updated questions on quiz:', error);
			throw error;
		}
	}
	...
	async getAllQuizzes() {
		try {
			...
			logger.error('database', 'Error fetching quizzes:', error);
			throw error;
		}
	}
	...
	async studentBelongsToTeacher(studentName: string, teacherName: string): Promise<boolean> {
		try {
			...
			logger.error('database', 'Error checking if student has teacher:', error);
			throw error;
		}
	}
}
